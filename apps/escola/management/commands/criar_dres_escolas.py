...
from escola.services.sme_integration import (
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        for dre in dres_qs:
            ...
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(
                        f"  Falha ao buscar UEs da DRE {dre.codigo}: {exc}"
                    )
                )
                continue
            self.stdout.write(
            ...
            for codigo_eol in codigos_ue:
                try:
                    ...
                    self.stderr.write(
                        self.style.ERROR(
                            f"  Falha ao buscar escola {codigo_eol}: {exc}"
                        )
                    )
                    continue
                ...
                logradouro_val = (
                    f"{tipo_logradouro} {logradouro_nome}".strip()
                    if tipo_logradouro
                ...
                try:
                    cep_val = (
                        int(cep_val_raw)
                        if cep_val_raw is not None
                        and str(cep_val_raw).strip() != ""
                        else 0
                    )
