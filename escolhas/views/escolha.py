...
from ..serializers import EscolhaListSerializer, EscolhaReconvocacaoSerializer, EscolhaSelectSerializer, EscolhaSerializer, EscolhasProdamImportacaoSerializer
...
class EscolhaViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = {'candidato_uuid': ['exact'], 'concurso_uuid': ['exact'], 'situacao': ['exact', 'in'], 'vaga_escola__cargo_codigo': ['exact'], 'vaga_escola__lote__processo_uuid': ['exact']}
    ...
    def busca(self, request: Any) -> Any:
        ...
        if not isinstance(candidato_ids, list):
            return Response({'detail': 'candidato_uuid deve ser uma lista de UUIDs.'}, status=status.HTTP_400_BAD_REQUEST)
        queryset = self.get_queryset().select_related('vaga_escola', 'vaga_escola__escola', 'vaga_escola__escola__dre').filter(candidato_uuid__in=candidato_ids)
    ...
    def buscar_candidatos(self, request: Any) -> Any:
        ...
        candidatos = CandidatoAPIService().buscar_candidatos(nome=nome or None, cpf=cpf or None, rg=rg or None, registro_funcional=registro_funcional or None)
        if candidatos is None:
            return Response({'detail': 'Erro ao consultar serviço de candidatos.'}, status=status.HTTP_400_BAD_REQUEST)
        codigos_cargo = set()
        ...
        logger.info('Candidatos encontrados', extra={'correlation_id': get_correlation_id(), 'method': request.method, 'path': request.path, 'params': request.query_params, 'user': request.user})
    ...
    def agrupar_por_cargo(self, request: Any) -> Any:
        ...
        data = {str(item['vaga_escola__cargo_codigo']): int(item['total'] or 0) for item in qs}
        logger.info('Agrupando escolhas por cargo - Resultado', extra={'correlation_id': get_correlation_id(), 'method': request.method, 'path': request.path, 'user': request.user, 'data': data})
    ...
    def importacao_prodam(self, request: Any) -> Any:
        ...
        if codigos_eol and codigos_cargo:
            try:
                vagas_escolas = VagasEscolas.objects.filter(escola__codigo_eol__in=codigos_eol, cargo_codigo__in=codigos_cargo).select_related('escola')
        ...
        for idx, escolha_data in enumerate(escolhas):
            try:
                ...
                if not candidato_uuid:
                    erros.append({'index': idx, 'cpf': cpf_escolha, 'erro': 'Candidato não encontrado'})
                    continue
                ...
                situacao_map = {'ESCOLHA': SituacaoChoices.ESCOLHA, 'NAO-ESCOLHA': SituacaoChoices.NAO_ESCOLHA, 'RECONVOCACAO': SituacaoChoices.RECONVOCACAO, 'PENDENTE': SituacaoChoices.NAO_ESCOLHA}
                ...
                if tipo_vaga_raw:
                    tipo_vaga_map = {'P': TipoVagaChoices.PRECARIA, 'D': TipoVagaChoices.DEFINITIVA}
                    tipo_vaga = tipo_vaga_map.get(str(tipo_vaga_raw).upper())
                escolha_existente = Escolha.objects.filter(candidato_uuid=candidato_uuid, concurso_uuid=concurso_uuid).first()
                if escolha_existente:
                    logger.warning(f'Escolha duplicada detectada na importação (índice {idx}): CPF={cpf_escolha}, candidato_uuid={candidato_uuid}, concurso_uuid={concurso_uuid}. Escolha existente UUID: {escolha_existente.uuid}')
                    continue
                nova_escolha = Escolha.objects.create(candidato_uuid=candidato_uuid, concurso_uuid=concurso_uuid, situacao=situacao, tipo_vaga=tipo_vaga, vaga_escola=vaga_escola)
                escolhas_criadas.append({'uuid': str(nova_escolha.uuid), 'candidato_uuid': str(candidato_uuid), 'situacao': nova_escolha.situacao})
            except Exception as exc:
                logger.error(f'Erro ao criar escolha {idx}: {exc}', exc_info=True)
                erros.append({'index': idx, 'cpf': escolha_data.get('cpf', 'N/A'), 'erro': str(exc)})
        response_data = {'escolhas_criadas': len(escolhas_criadas), 'escolhas': escolhas_criadas}
        if erros:
            logger.info('Erros ao criar escolhas', extra={'correlation_id': get_correlation_id(), 'erros': erros})
            response_data['erros'] = erros
            return Response(response_data, status=status.HTTP_400_BAD_REQUEST)
        logger.info('Escolhas criadas', extra={'correlation_id': get_correlation_id(), 'escolhas_criadas': len(escolhas_criadas), 'escolhas': escolhas_criadas[:10]})
