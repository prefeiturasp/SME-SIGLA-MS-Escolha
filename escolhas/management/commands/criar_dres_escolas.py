...
from escolhas.services.sme_integration import buscar_dados_escola_por_eol, buscar_dres_de_smeintegracao, buscar_ues_codigos_por_dre
...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        self.stdout.write(self.style.SUCCESS(f'✅ Concluído: criadas={created_count}, atualizadas={updated_count}, sem_alteracao={skipped_count}'))
        ...
        for dre in dres_qs:
            ...
            for codigo_eol in codigos_ue:
                ...
                try:
                    ...
                    cep_val = 0
                escola_defaults = {'dre': dre, 'nome_oficial': (dados.get('nome') or '').strip() or 'DESCONHECIDO', 'nome_nao_oficial': (dados.get('nomeExibicao') or '').strip() or None, 'tipo_unidade_admin': tipo_unidade_admin_val, 'tipo_ue': tipo_ue_val, 'logradouro': logradouro_val or 'DESCONHECIDO', 'numero': numero_val or 'S/N', 'bairro': bairro_val or 'DESCONHECIDO', 'cep': cep_val, 'distrito': 'DESCONHECIDO', 'sub_prefeitura': 'DESCONHECIDA', 'nome_dre': nome_dre, 'email': (dados.get('email') or '').strip() or None, 'telefone1': (dados.get('telefone') or '').strip() or None, 'telefone2': None, 'ano_construcao': None, 'propriedade': 'DESCONHECIDO', 'capacidade_vagas_matutino': 0, 'capacidade_vagas_vespertino': 0, 'capacidade_vagas_noturno': 0, 'capacidade_vagas_intermediario': 0, 'capacidade_vagas_integral': 0, 'capacidade_vagas_total': 0, 'organizacao_parceira': False, 'quantidade_de_funcionarios': 0, 'status': 'ATIVA'}
                escola, created = Escola.objects.get_or_create(codigo_eol=str(dados.get('codigo') or codigo_eol), defaults=escola_defaults)
                if created:
                    escolas_criadas += 1
        ...
        self.stdout.write(self.style.SUCCESS(f'✅ Escolas: criadas={escolas_criadas}, atualizadas={escolas_atualizadas}, sem_alteracao={escolas_sem_alteracao}'))
