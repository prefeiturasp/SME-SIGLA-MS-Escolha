"""Comando para atualizar o campo codigo_integracao das escolas a partir da API.
...
from escolhas.services.sme_integration import buscar_dres_de_smeintegracao, buscar_unidades_codigo_integracao_por_dre
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        with transaction.atomic():
            for item in dres:
                ...
                self.stdout.write(self.style.HTTP_INFO(f'Consultando unidades da DRE {sigla_dre} ({codigo_dre})...'))
                try:
                    unidades = buscar_unidades_codigo_integracao_por_dre(codigo_dre)
                except Exception as exc:
                    self.stderr.write(self.style.ERROR(f'  Falha ao buscar unidades da DRE {codigo_dre}: {exc}'))
                    erros += 1
                    continue
                ...
                for u in unidades:
                    ...
                    try:
                        ...
                        nao_encontradas += 1
                        self.stdout.write(self.style.WARNING(f'  Escola não encontrada no BD: codigo_eol={codigo_ue}'))
                        continue
                    if codigo_integracao and codigo_integracao != '' and (escola.codigo_integracao != codigo_integracao):
                        escola.codigo_integracao = codigo_integracao
                        escola.save(update_fields=['codigo_integracao'])
                        atualizadas += 1
                        self.stdout.write(f'  ↻ Atualizada: {escola.nome_oficial} ({codigo_ue}) -> {codigo_integracao}')
        self.stdout.write(self.style.SUCCESS(f'✅ Concluído: atualizadas={atualizadas}, não encontradas={nao_encontradas}, erros_dre={erros}'))
