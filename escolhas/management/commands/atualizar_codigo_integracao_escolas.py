"""
Comando para atualizar o campo codigo_integracao das escolas a partir da API SME.
1. Busca DREs na API.
2. Para cada DRE, chama GET /api/DREs/{dreCodigo}/unidades/codigo-integracao.
3. Para cada unidade retornada, localiza a escola pelo codigo_eol (codigoUe) e atualiza codigo_integracao.
"""
...
class Command(BaseCommand):
    help = (
        ...
        'para cada DRE obtém unidades (codigo-integracao) e atualiza as escolas pelo codigo_eol.'
    )
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for item in dres:
                ...
                nome_dre = item.get('nome', '')
                sigla_dre = item.get('sigla', '')

                self.stdout.write(
                    self.style.HTTP_INFO(
                        f'Consultando unidades da DRE {sigla_dre} ({codigo_dre})...'
                    )
                )
                ...
                try:
                    ...
                    self.stderr.write(
                        self.style.ERROR(f'  Falha ao buscar unidades da DRE {codigo_dre}: {exc}')
                    )
                    erros += 1
                    continue
                ...
                for u in unidades:
                    ...
                    try:
                        ...
                        nao_encontradas += 1
                        self.stdout.write(
                            self.style.WARNING(f'  Escola não encontrada no BD: codigo_eol={codigo_ue}')
                        )
                        continue
                    ...
                    if codigo_integracao and codigo_integracao != '' and escola.codigo_integracao != codigo_integracao:
                        ...
                        atualizadas += 1
                        self.stdout.write(
                            f'  ↻ Atualizada: {escola.nome_oficial} ({codigo_ue}) -> {codigo_integracao}'
                        )

        self.stdout.write(
            self.style.SUCCESS(
                f'✅ Concluído: atualizadas={atualizadas}, não encontradas={nao_encontradas}, erros_dre={erros}'
            )
        )
