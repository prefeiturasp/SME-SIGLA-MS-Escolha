...
class CandidatoAPIService:
    ...
    def buscar_candidatos_por_cpfs(self, cpfs: list[str], processo_uuid: str) -> str | None:
        ...
        logger.info('Buscando candidatos por CPFs', extra={'method': 'POST', 'correlation_id': get_correlation_id(), 'url': url, 'processo_uuid': processo_uuid, 'cpfs': cpfs, 'headers': self._default_headers})
        try:
            ...
            return None
        data = response.json()
        logger.info('Candidatos encontrados', extra={'correlation_id': get_correlation_id(), 'method': 'POST', 'url': url, 'processo_uuid': processo_uuid, 'cpfs': cpfs})
    ...
    def buscar_candidatos(self, nome: str | None=None, cpf: str | None=None, rg: str | None=None, registro_funcional: str | None=None) -> list[dict] | None:
        ...
        logger.info('Buscando candidatos no MS-Candidatos', extra={'correlation_id': get_correlation_id(), 'url': url, 'nome': nome, 'cpf': cpf, 'rg': rg, 'registro_funcional': registro_funcional, 'method': 'GET', 'headers': self._default_headers})
        params = {}
        ...
        try:
            ...
            return None
        logger.info('Candidatos encontrados', extra={'correlation_id': get_correlation_id(), 'url': url, 'nome': nome, 'cpf': cpf, 'rg': rg, 'registro_funcional': registro_funcional, 'method': 'GET', 'headers': self._default_headers, 'status_code': response.status_code, 'response': str(response.json())[:100]})
