...
class TestCandidatoAPIService:
    ...
    def test_buscar_candidatos_por_cpfs_lista_vazia(
        self, settings: Any
    ) -> None:
        ...
        processo_uuid = "123e4567-e89b-12d3-a456-426614174000"
        mock_response_data = []  # type: ignore[var-annotated]
        with patch("sigla_sdk.http.api_client.http_client.post") as mock_post:
            mock_response = Mock()
            mock_response.json.return_value = mock_response_data
            mock_response.raise_for_status.return_value = None
    ...
    def test_buscar_candidatos_por_cpfs_http_error(
        self, settings: Any
    ) -> None:
    ...
    def test_buscar_candidatos_por_cpfs_connection_error(
        self, settings: Any
    ) -> None:
    ...
    def test_buscar_candidatos_por_cpfs_excecao_generica(
        self, settings: Any
    ) -> None:
    ...
    def test_buscar_candidatos_por_cpfs_timeout_personalizado(
        self, settings: Any
    ) -> None:
    ...
    def test_buscar_candidatos_por_cpfs_headers_corretos(
        self, settings: Any
    ) -> None:
    ...
    def test_buscar_candidatos_sem_parametros_retorna_lista_vazia(
        ...
        """Sem nenhum parâmetro informado, deve retornar lista vazia (não."""
        settings.CANDIDATOS_API_URL = "http://test-api.com"
        service = CandidatoAPIService()
        with patch("sigla_sdk.http.api_client.http_client.get") as mock_get:
    ...
    def test_buscar_candidatos_envia_params_corretos(
        self, settings: Any
    ) -> None:
        """Testa que nome, cpf, rg e registro_funcional são enviados como."""
        settings.CANDIDATOS_API_URL = "http://test-api.com"
        service = CandidatoAPIService()
        with patch("sigla_sdk.http.api_client.http_client.get") as mock_get:
            mock_response = Mock()
    ...
    def test_buscar_candidatos_http_error_retorna_none(
        self, settings: Any
    ) -> None:
    ...
    def test_buscar_candidatos_connection_error_retorna_none(
        self, settings: Any
    ) -> None:
