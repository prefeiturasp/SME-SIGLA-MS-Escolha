import pytest
from escolhas.models import Parametrizacao, Escola, Dre
...
def test_sync_from_escolas_ignora_vazios_e_respeita_existentes():
    ...
    # Escola com tipo_ue vazio deve ser ignorada
    _criar_escola(dre, '200003', 'Escola C', '')

    created = Parametrizacao.sync_from_escolas()
    assert created == 1  # apenas EMEI criada, EMEF já existia, vazio ignorado
    tipos = set(Parametrizacao.objects.values_list('tipo_ue', flat=True))
    assert tipos == {'EMEF', 'EMEI'}
