...
class EscolhaSerializer(DynamicFieldsSerializer):
    ...
    vaga_escola_uuid = serializers.UUIDField(write_only=True, required=False, allow_null=True, help_text='UUID da vaga da escola. Será convertido para ForeignKey internamente.')

    class Meta:
        """Define Meta."""
        model = Escolha
        fields = ['uuid', 'candidato_uuid', 'concurso_uuid', 'situacao', 'tipo_vaga', 'e_retardatario', 'vaga_escola_uuid', 'vaga_escola', 'criado_em', 'atualizado_em']
    ...
    def validate(self, attrs: Any) -> Any:
        ...
        if vaga_escola_uuid and (not vaga_escola):
            try:
                ...
                raise serializers.ValidationError({'vaga_escola_uuid': f'VagaEscola com UUID {vaga_escola_uuid} não encontrada.'})
        attrs.pop('vaga_escola_uuid', None)
        if situacao == SituacaoChoices.ESCOLHA:
            if not tipo_vaga:
                raise serializers.ValidationError({'tipo_vaga': 'Este campo é obrigatório quando a situação é "escolha".'})
            if not vaga_escola:
                raise serializers.ValidationError({'vaga_escola_uuid': 'Este campo é obrigatório quando a situação é "escolha".'})
        return attrs
