...
class VagasEscolasSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        """Define Meta."""
        model = VagasEscolas
        fields = ['uuid', 'lote_uuid', 'data_fechamento_modulo', 'cargo_codigo', 'cargo_descricao', 'vagas_precarias', 'vagas_precarias_utilizadas', 'vagas_precarias_restantes', 'vagas_definitivas', 'vagas_definitivas_utilizadas', 'vagas_definitivas_restantes', 'foi_utilizada', 'esta_checada', 'status', 'escola', 'criado_em', 'atualizado_em']
        read_only_fields = ['uuid', 'criado_em', 'atualizado_em']

class VagasEscolasCreateSerializer(serializers.Serializer):
    """Serializer para criação de vagas em lote."""
    processo_uuid = serializers.UUIDField(error_messages={'invalid': 'Deve ser um UUID válido.', 'required': 'Este campo é obrigatório.', 'null': 'Este campo não pode ser nulo.'})
    ...
    def validate_vagas(self, value: Any) -> Any:
        ...
        for i, vaga in enumerate(value):
            required_fields = ['data_fechamento_modulo', 'cargo_codigo', 'cargo_descricao', 'codigo_eol', 'vagas_precarias', 'vagas_definitivas', 'status']
            for field in required_fields:
                if field not in vaga:
                    raise serializers.ValidationError(f"Campo '{field}' é obrigatório na vaga {i + 1}.")
        return value
...
class VagasEscolasUtilizadasUpdateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        """Define Meta."""
        model = VagasEscolas
        fields = ['vagas_precarias_utilizadas', 'vagas_definitivas_utilizadas', 'foi_utilizada', 'esta_checada']
