...
_core_urlpatterns = [
    path("api/v1/", include("escola.api.urls")),
    path("api/v1/", include("parametrizacao.api.urls")),
    path("api/v1/", include("vagas_escolas.api.urls")),
    path("api/v1/", include("escolhas.api.urls")),
...
if getattr(settings, "DJANGO_ENVIRONMENT", "local") != "local":
    _ms_prefix = getattr(settings, "MS_PATH", "/ms-escolha").strip("/")
    urlpatterns = [
